...
export default function AdminPage() {
  ...
  const loadShiftReport = async (shiftId: string) => {
    ...
    const [
      ...
      supabase.from('write_offs').select('*, products(name), users(name)').eq('shift_id', shiftId),
      supabase.from('orders').select('id, pay_type, worker_id').eq('shift_id', shiftId),
      supabase.from('orders').select('id, pay_type, worker_id, created_at').eq('shift_id', shiftId).eq('pay_type', 'ichki'),
    ])

    let orderItemsData: any[] = []
    if (ords && ords.length > 0) {
      const orderIds = ords.map((o: any) => o.id)
      const { data: oi } = await supabase.from('order_items').select('*, products(name)').in('order_id', orderIds)
      orderItemsData = oi || []
    }
    ...
    const enriched = (report || []).map((r: any) => {
      ...
      const smenaChiqim = woData?.filter((w: any) => w.product_id === r.product_id && w.reason === 'Smena boshida chiqindi').reduce((s: number, w: any) => s + w.qty, 0) || 0
      const oddiyChiqim = woData?.filter((w: any) => w.product_id === r.product_id && w.reason !== 'Smena boshida chiqindi').reduce((s: number, w: any) => s + w.qty, 0) || 0
      const totalChiqim = oddiyChiqim + smenaChiqim
      const realInitial = initial + smenaChiqim
      let kirim = 0
      if (prod?.unit_type === 'hissa') {
        const kgKirim = siData?.filter((si: any) => si.product_id === r.product_id).reduce((s: number, si: any) => s + si.qty, 0) || 0
        if (kgKirim > 0) {
          const hissaKirim = Math.round(kgKirim * (prod.kg_to_hissa || 21))
          kirim = Math.floor(hissaKirim / (prod.hissa_per_unit || 3))
        }
        return { ...r, initial: realInitial, kirim, sotildi, chiqim: totalChiqim, qoldiq: r.actual_qty }
      }
      kirim = siData?.filter((si: any) => si.product_id === r.product_id).reduce((s: number, si: any) => s + si.qty, 0) || 0
    ...
    izohlar.sort((a, b) => {
      if (!a.vaqt) return 1
      if (!b.vaqt) return -1
      return new Date(a.vaqt).getTime() - new Date(b.vaqt).getTime()
    })

    setShiftReports(prev => ({ ...prev, [shiftId]: { mahsulot: enriched, izohlar } }))
  }
  ...
  const soldByProduct = products.map(p => {
    ...
    const smenaChiqim = writeOffs.filter(w => w.product_id === p.id && shift && new Date(w.created_at) >= new Date(shift.opened_at) && w.reason === 'Smena boshida chiqindi').reduce((s, w) => s + w.qty, 0)
    const oddiyChiqim = writeOffs.filter(w => w.product_id === p.id && shift && new Date(w.created_at) >= new Date(shift.opened_at) && w.reason !== 'Smena boshida chiqindi').reduce((s, w) => s + w.qty, 0)
  ...
  const generatePDF = () => {
    ...
    doc.text('Moliyaviy korsatkichlar:', 20, y); y += 8
    doc.setFontSize(10); doc.setFont('helvetica', 'normal')
    const stats = [
      ...
      ['Ichki istemol:', `${ichkiOrders.length} ta`],
    ]
    ...
    <div style={{minHeight:'100vh', backgroundColor:'#F5F3EE', paddingBottom:'80px'}}>
      ...
      <div style={{padding:'16px', maxWidth:'672px', margin:'0 auto'}}>
        ...
        {activeTab === 'hisobot' && (
          <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px'}}>
              {[
                { label: 'Jami tushum', val: fmt(actualRev) + ' som', color: '#C8860A', sub: totalRev !== actualRev ? `Narx: ${fmt(totalRev)}` : undefined },
                { label: 'Buyurtmalar', val: shiftOrders.length + ' ta', color: '#1A1208' },
                { label: 'Naqd', val: fmt(cashRev) + ' som', color: '#1E7B47' },
                { label: 'Click', val: fmt(clickRev) + ' som', color: '#9333ea' },
            ...
            {moslashuvOrders.length > 0 && (
              <div style={{backgroundColor:'white', borderRadius:'16px', border:'1px solid #e5e7eb', overflow:'hidden'}}>
                <div style={{padding:'12px 16px', borderBottom:'1px solid #f3f4f6', fontWeight:900, fontSize:'14px'}}>Moslashuvchan tolovlar</div>
                {moslashuvOrders.map(o => (
                  <div key={o.id} style={{padding:'12px 16px', borderBottom:'1px solid #f9fafb'}}>
                    <div style={{display:'flex', justifyContent:'space-between', fontSize:'14px'}}>
                      <span style={{color:'#6b7280'}}>Narx: {fmt(o.total)} som</span>
                      <span style={{fontWeight:700, color:'#C8860A'}}>Tolandi: {fmt(o.actual_paid || o.total)} som</span>
                    </div>
            ...
            {oshHissa > 0 && (
              <div style={{backgroundColor:'white', borderRadius:'16px', border:'1px solid #e5e7eb', overflow:'hidden'}}>
                <div style={{padding:'12px 16px', borderBottom:'1px solid #f3f4f6', fontWeight:900, fontSize:'14px'}}>Osh qoldigi</div>
            ...
            {ichkiOrders.length > 0 && (
              <div style={{backgroundColor:'white', borderRadius:'16px', border:'1px solid #e5e7eb', overflow:'hidden'}}>
                <div style={{padding:'12px 16px', borderBottom:'1px solid #f3f4f6', fontWeight:900, fontSize:'14px', color:'#f97316'}}>Ichki istemol</div>
                {ichkiOrders.map(o => {
                  ...
                  return (
                    <div key={o.id} style={{padding:'12px 16px', borderBottom:'1px solid #f9fafb'}}>
                      ...
                      <div style={{fontSize:'12px', color:'#6b7280'}}>{items.map(i => `${productName(i.product_id)} x ${i.qty}`).join(', ')}</div>
                    </div>
                  )
                })}
              </div>
        ...
        {activeTab === 'tarix' && (
          <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
            ...
            {shiftHistory.length === 0 ? (
              ...
              const izohlar = reportData?.izohlar || []
                ...
                <div key={s.id} style={{backgroundColor:'white', borderRadius:'16px', border:'1px solid #e5e7eb', overflow:'hidden'}}>
                  <div style={{padding:'12px 16px', cursor:'pointer'}}
                    ...
                    <div style={{display:'flex', alignItems:'center', gap:'12px', marginLeft:'16px'}}>
                      ...
                      <span style={{fontWeight:900, fontSize:'14px', color:'#C8860A'}}>{fmt(s.total_revenue)} som</span>
                    ...
                    <div style={{borderTop:'1px solid #f3f4f6', padding:'12px 16px', backgroundColor:'#f9fafb', display:'flex', flexDirection:'column', gap:'12px'}}>
                      ...
                      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px'}}>
                        {[
                          ...
                          { label: 'Naqd', val: fmt(s.cash_revenue) + ' som', color: '#16a34a' },
                          { label: 'Click', val: fmt(s.click_revenue) + ' som', color: '#9333ea' },
                          { label: 'Karta', val: fmt(s.card_revenue) + ' som', color: '#2563eb' },
                          { label: 'Qarz', val: fmt(s.debt_revenue) + ' som', color: '#dc2626' },
                      ...
                      {izohlar.length > 0 && (
                        <div style={{backgroundColor:'white', borderRadius:'12px', overflow:'hidden'}}>
                          <div style={{padding:'8px 12px', borderBottom:'1px solid #f3f4f6', fontWeight:900, fontSize:'12px', color:'#6b7280'}}>
                            Izohlar va hodisalar ({izohlar.length} ta)
                          </div>
                          {izohlar.map((iz: any, i: number) => (
                            <div key={i} style={{padding:'8px 12px', borderBottom:'1px solid #f9fafb', display:'flex', gap:'8px', alignItems:'flex-start'}}>
                              <div style={{fontSize:'14px', flexShrink:0, marginTop:'1px'}}>
                                {iz.type === 'chiqim' ? '⚠️' : iz.type === 'ichki' ? '🍽️' : iz.type === 'smena_chiqim' ? '🗑️' : '📊'}
                              </div>
                              <div style={{flex:1, minWidth:0}}>
                                <div style={{fontSize:'12px', fontWeight:700, color:'#1A1208'}}>{iz.text}</div>
                                <div style={{fontSize:'11px', color:'#C8860A', fontStyle:'italic', marginTop:'2px'}}>"{iz.izoh}"</div>
                                <div style={{display:'flex', gap:'8px', marginTop:'2px', fontSize:'10px', color:'#9ca3af'}}>
                                  <span>{iz.xodim}</span>
                                  {iz.vaqt && <span>{new Date(iz.vaqt).toLocaleTimeString('uz-UZ', {hour:'2-digit', minute:'2-digit'})}</span>}
                                </div>
                              </div>
                            </div>
                          ))}
        ...
        {activeTab === 'mahsulot' && (
          <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
            <div style={{backgroundColor:'white', borderRadius:'16px', border:'1px solid #e5e7eb', padding:'16px'}}>
              <div style={{fontWeight:900, fontSize:'14px', marginBottom:'12px'}}>Kategoriya qoshish</div>
              ...
              <div style={{display:'flex', flexWrap:'wrap', gap:'8px', marginTop:'12px'}}>
                {categories.map(c => (
                  <div key={c.id} style={{display:'flex', alignItems:'center', gap:'4px', backgroundColor:'#FFF8E7', border:'1px solid #F5C842', borderRadius:'999px', padding:'4px 12px'}}>
                    ...
                    <button onClick={() => deleteCategory(c.id)} style={{color:'#9ca3af', background:'none', border:'none', cursor:'pointer', fontSize:'12px', marginLeft:'4px'}}>x</button>
                  </div>
                ))}
              </div>
            </div>
                ...
                return (
                  <div key={p.id} style={{display:'flex', alignItems:'center', gap:'12px', padding:'12px 16px', borderBottom:'1px solid #f9fafb'}}>
                    <div style={{flex:1, minWidth:0}}>
                      ...
                      <div style={{fontSize:'12px', color:'#9ca3af'}}>{cat?.name} - {fmt(p.price)} som</div>
                    </div>
                    <button onClick={() => toggleAvail(p)}
                      ...
                      {p.is_available ? 'Bor' : "Yoq"}
                    </button>
                    <button onClick={() => deleteProduct(p.id)} style={{color:'#d1d5db', background:'none', border:'none', cursor:'pointer', fontSize:'18px', flexShrink:0}}>x</button>
                  </div>
                )
              })}
            </div>
        ...
        {activeTab === 'xodim' && (
          <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
            <div style={{backgroundColor:'white', borderRadius:'16px', border:'1px solid #e5e7eb', padding:'16px'}}>
              <div style={{fontWeight:900, fontSize:'14px', marginBottom:'12px'}}>Xodim qoshish</div>
              <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
            ...
            <div style={{backgroundColor:'white', borderRadius:'16px', border:'1px solid #e5e7eb', overflow:'hidden'}}>
              ...
              {workers.map(w => (
                <div key={w.id} style={{display:'flex', alignItems:'center', gap:'12px', padding:'12px 16px', borderBottom:'1px solid #f9fafb'}}>
                  ...
                  <button onClick={() => deleteWorker(w.id)} style={{color:'#d1d5db', background:'none', border:'none', cursor:'pointer', fontSize:'18px'}}>x</button>
                </div>
              ))}
            </div>
          </div>
